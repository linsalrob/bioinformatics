...
foreach my $file (@ARGV)
{
	...
	while (my $seq=$sio->next_seq) {
		...
		my @seqdata = ( $seq->display_name );
		if (defined $seq->seq_version()) {$seqdata[0] .= ".".$seq->seq_version()}
		push @seqdata, $seq->length();
		push @seqdata, $seq->desc();
		push @seqdata, $seq->primary_id();
