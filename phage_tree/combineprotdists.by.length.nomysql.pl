...
# this loop will take care of that.
			...
			if ($oldmatrix[$y][$x] || $oldmatrix[$x][$x]) {
				...
				eval {$oldmatrix[$y][$x] = $oldmatrix[$x][$y] = $oldmatrix[$y][$x]/$oldcount[$y][$x]};
				if ($@) {$oldmatrix[$y][$x] = $oldmatrix[$x][$y] = 0}
...
# this is just to get around various error messages
					...
					eval {$oldmatch[$y][$x] = $oldmatch[$x][$y] = 
							($oldmatch[$y][$x]+$oldmatch[$x][$y])/($oldmatchcount[$y][$x]+$oldmatchcount[$x][$y]);};
					if ($@) {$oldmatch[$y][$x] = $oldmatch[$x][$y] = 0}

					eval {$oldmismatch[$y][$x] = $oldmismatch[$x][$y] = 
							($oldmismatch[$y][$x]+$oldmismatch[$x][$y])/($oldmismatchcount[$y][$x]+$oldmismatchcount[$x][$y]);};
					if ($@) {$oldmismatch[$y][$x] = $oldmismatch[$x][$y] = 0}

					eval {$newmatch[$y][$x] = $newmatch[$x][$y] = 
							($newmatch[$y][$x]+$newmatch[$x][$y])/($newmatchcount[$y][$x]+$newmatchcount[$x][$y]);};
					if ($@) {$newmatch[$y][$x] = $newmatch[$x][$y] = 0}

					eval {$newmismatch[$y][$x] = $newmismatch[$x][$y] = 
							($newmismatch[$y][$x]+$newmismatch[$x][$y])/($newmismatchcount[$y][$x]+$newmismatchcount[$x][$y]);};
					if ($@) {$newmismatch[$y][$x] = $newmismatch[$x][$y] = 0}
