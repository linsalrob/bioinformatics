...
print STDERR "Adding $genome\n";
	...
	$templateFrame{$genome}->string(-coords => ["-${offset}p", '0p'], -text => $$organismname{$genome}, -color => 'navy');
	}
	...
	$templateFrame{$genome}->line(-coords=>['0', '1', '1', '1'], -fill=>'black', -width=>'0.01');
