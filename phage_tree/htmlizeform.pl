...
my $protmatchfile = shift || print STDERR "WARNING: No protein match file written. \"Changeme\" written instead\n";
unless ($protmatchfile) {$protmatchfile= "Changeme"}
...
while (my @retrieved = $exc->fetchrow_array) {
	$genomename{$retrieved[0]} = $retrieved[1]." (".$retrieved[2].")";
	if ($retrieved[0]>$max) {$max=$retrieved[0]}
	}

open (IN, $treefile) || &niceexit("Can't open $treefile\n");
open (OUT, ">$treefile.form.html")  || &niceexit("Can't open $treefile.corrected for writing\n");
my $table;
print OUT "<html><title>Phage Trees</title></head><body bgcolor=\"#FFFFFF\">\n<pre>\n"; 
print OUT "<FORM ENCTYPE=\"multipart/form-data\" ACTION=\"/cgi-bin/getproteinmatchesbyfile.cgi\" METHOD=\"POST\">\n";
print OUT "<center><input type=submit><input type=reset></center>\n<input type=hidden name=\"file\" value=\"$protmatchfile\">\n";


while (<IN>) {
	s/genome(\d+)/$genomename{$1}<input type=checkbox name=$1 value=$1>/g;
...
print OUT "<center><input type=submit><input type=reset></center>\n<input type=hidden name=max value=$max></form>\n";
print OUT "</body></html>\n";
