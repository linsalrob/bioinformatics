...
if ($pad) {
   ...
   if ($args =~ /-l/) {$genomeproteins = &getallprots()}
...
if ($verb) {if ($y + $x ==19) {print "xxsimilar $y, $x -> ", $#similar+1, ":\n|", join ("|\n|", @similar), "|\n"}}
...
foreach my $y (0 .. $#matrix) {
   ...
   foreach my $x (1 .. $#{$matrix[$y]}) {
      ...
      if (($oldmatrix[$y][$x] || $oldmatrix[$x][$x]) && ($oldcount[$y][$x] || $oldcount[$x][$y])) {
           # this is just to get around various error messages
           unless ($oldmatrix[$y][$x]) {$oldmatrix[$y][$x]=0}
           unless ($oldmatrix[$x][$y]) {$oldmatrix[$x][$y]=0}
           unless ($oldcount[$y][$x]) {$oldcount[$y][$x]=0}
           unless ($oldcount[$x][$y]) {$oldcount[$x][$y]=0}
