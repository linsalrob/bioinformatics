...
$usage .= "\t-n DON'T pad missing proteins with worst score (100 or penalty)\n\t-m print out all protein matches\n";
...
$usage .= "\t-sh print out self hits\n\t-l print length data for groups\n";
$usage .= "\t-s # skip scores above this number\n";
$usage .= "\t-p penalty\n\t-v verbose output";
...
if ($args =~ /-n/) {$pad=0}
my $skip=0;
if ($args =~ /-s\s+(\d+)/) {$skip=$1}
my $penalty=100;
if ($args =~ /-p\s+(\d+)/) {$penalty=$1}
my $verb;
if ($args =~ /-v/) {$verb=1}
print STDERR "Using penalty of $penalty and skip of $skip\n";
...
#print STDERR "\nSKIPPING: ";
		...
		foreach my $x (1 .. $#genomes) {
			...
			# check for the skip value, and make it $pad or ignore
			if ($line[$x] > $skip) {if ($pad) {$line[$x] = $penalty} else {next}}
			...
			if (($genomes[$x] == $linegenome) && !($args =~ /-sh\W/)) {$line[$x] = '0.000'}
			else {
				my $genematch;
				...
				unless ($line[$x] == $penalty) {push (@{$allscores[$linegenome][$genomes[$x]]}, $line[$x])}
...
if ($verb) {print STDERR "$y, $x: Mean: $matrixsum[$x][$y] STDEV: $stdev[$y][$x] FROM $stdevcount[$y][$x]. Total so far = $totalprots\n"}
	}
   }
...
sub getlength {
  ...
  my %long; my %short;
  foreach my $key (keys %$group) {
    unless (exists $short{$$group{$key}}) {$short{$$group{$key}} = 1000000000000}
    unless (exists $long{$$group{$key}}) {$long{$$group{$key}} = 1}
    my $exc = $dbh->prepare("select beginning, end from phage where count=$key");
    $exc->execute or die $dbh->errstr;
    while (my @ret = $exc->fetchrow_array) {
      my $length = abs($ret[0]-$ret[1]);
      if ($length > $long{$$group{$key}}) {$long{$$group{$key}}= $length}
      if ($length < $short{$$group{$key}}) {$short{$$group{$key}} = $length}
    }
 }
open (OUT, ">groups.min.max.length.txt") || die "can't open groups.min.max.length.txt for writing\n";
print OUT "Group\tshortest\tlongest\n";

foreach my $key (sort {$a <=> $b} keys %long) {print OUT "$key\t$short{$key}\t$long{$key}\n"}  
close OUT;
}
