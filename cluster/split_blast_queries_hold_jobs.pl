...
-p blast program
	-db blast database
-ex blast executable (default is /usr/local/bin/blastall)

-N job name (default is blastfile) 

-h script to submit with a hold_jid on this job. You can supply more than one -h command (eg. -h script1.sh -h script2.sh)
...
my $blastopt=" ";
my ($blastexec) = ('/usr/local/bin/blastall');
my @holdjobs;
...
while (@ARGV) {
	...
	elsif ($test eq "-h") {push @holdjobs, shift @ARGV}
...
if (!$jobid) {
	print STDERR "Job id was not found, we can not execute these scripts. Sorry\n";
}

foreach my $script (@holdjobs) {
	if (-e $script) {
		if (-x $script) {
			print STDERR "Submitting $script with a hold\n";
			print `qsub -S /bin/bash -cwd -e sge_output -o sge_output -hold_jid $jobid $script`;
		} else {
			print STDERR "$script was found but is not executable\n";
		}
	}
	else {
		print STDERR "$script was not found and can not be submitted\n";
	}
}
